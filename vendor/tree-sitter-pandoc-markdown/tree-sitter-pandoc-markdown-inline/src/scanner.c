...
typedef enum {
    LATEX_DELIMITER_NONE = 0,
    LATEX_DELIMITER_DOLLAR = 1,
    LATEX_DELIMITER_PAREN = 2,
    LATEX_DELIMITER_BRACKET = 3
} LatexDelimiterKind;
...
typedef struct {
    ...
    uint8_t latex_span_delimiter_kind;
...
static unsigned serialize(Scanner *s, char *buffer) {
    ...
    buffer[size++] = (char)s->latex_span_delimiter_kind;
...
static void deserialize(Scanner *s, const char *buffer, unsigned length) {
    ...
    s->latex_span_delimiter_kind = LATEX_DELIMITER_NONE;
    ...
    if (length > 0) {
        ...
        if (length > size) {
            s->latex_span_delimiter_kind = (uint8_t)buffer[size++];
        }
        if (length > size) {
...
static bool parse_dollar(Scanner *s, TSLexer *lexer,
                         const bool *valid_symbols) {
    if (s->latex_span_delimiter_kind != LATEX_DELIMITER_NONE &&
        s->latex_span_delimiter_kind != LATEX_DELIMITER_DOLLAR) {
        return false;
    }

    bool parsed = parse_leaf_delimiter(lexer, &s->latex_span_delimiter_length,
    ...
    if (lexer->result_symbol == LATEX_SPAN_START) {
        s->latex_span_delimiter_kind = LATEX_DELIMITER_DOLLAR;
    } else if (lexer->result_symbol == LATEX_SPAN_CLOSE ||
               lexer->result_symbol == UNCLOSED_SPAN) {
        s->latex_span_delimiter_kind = LATEX_DELIMITER_NONE;
    }
...
static bool parse_backslash_latex(Scanner *s, TSLexer *lexer,
                                  const bool *valid_symbols) {
    lexer->advance(lexer, false);

    LatexDelimiterKind delimiter_kind = LATEX_DELIMITER_NONE;
    int32_t close_char = 0;
    bool is_open = false;

    switch (lexer->lookahead) {
        case '(':
            delimiter_kind = LATEX_DELIMITER_PAREN;
            close_char = ')';
            is_open = true;
            break;
        case '[':
            delimiter_kind = LATEX_DELIMITER_BRACKET;
            close_char = ']';
            is_open = true;
            break;
        case ')':
    ...
    if (!is_open) {
        if (s->latex_span_delimiter_kind != delimiter_kind ||
            !valid_symbols[LATEX_SPAN_CLOSE]) {
            return false;
        }
    ...
    while (!lexer->eof(lexer)) {
        if (lexer->lookahead == '\\') {
            ...
            if (lexer->lookahead == close_char) {
                s->latex_span_delimiter_length = 2;
                s->latex_span_delimiter_kind = delimiter_kind;
...
static bool scan(Scanner *s, TSLexer *lexer, const bool *valid_symbols) {
    ...
    switch (lexer->lookahead) {
        ...
        case '\\':
            return parse_backslash_latex(s, lexer, valid_symbols);
        case '*':
